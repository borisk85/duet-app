...
DETAIL_LEVEL_MAP = {
    "simple":   "РЕЖИМ ПРОСТО: это совет другу который НИЧЕГО не знает об алкоголе. Никакой посуды, никаких температур, никаких действий требующих специального оборудования или знаний (никаких бокалов, декантеров, термометров, градусов, аэрации, охлаждения посуды). Только: ЧТО купить, КАК это называется, и ОДНО бытовое действие из разрешённых — 'пить холодным', 'открыть заранее', 'налить и всё', 'пить после еды'. Reason — РОВНО одно предложение до 10 слов на бытовом языке. Serving_tip — одна короткая бытовая фраза из разрешённых выше. НИКАКИХ терминов ('танины', 'кислотность', 'минеральность', сорта винограда и т.д.). Если не уверен можно ли слово — его НЕЛЬЗЯ.",
    ...
    "expert":   "РЕЖИМ ЭКСПЕРТ: профессиональное описание для знатока который читает Decanter и ходит в винотеки. Reason — 2-3 предложения с глубокой информацией: сорт винограда (можно использовать оригинальные международные названия 'Cabernet Sauvignon', 'Pinot Noir', 'Sangiovese'), регион происхождения с подробностями ('Bordeaux, Левый берег', 'Barolo, Пьемонт', 'Rioja Alta'), выдержка ('18 месяцев в дубе', '5 лет в бочках из-под бурбона'), стилистика производителя. Обосновывай парность через вкусовой профиль — танины, кислотность, минеральность, фруктовость, дубовые ноты, длина послевкусия. Serving_tip — точная температура подачи в °C, тип бокала ('Bordeaux universal', 'Burgundy balloon', 'Glencairn для виски'), декантация/аэрация/время дыхания если применимо. ОБЯЗАТЕЛЬНО включи поле 'why_it_works' — отдельное короткое (1-2 предложения) объяснение гастрономической логики сочетания. Не дублируй reason — здесь именно ПРИНЦИП почему два продукта работают вместе на уровне химии вкуса. Примеры: 'Танины красного вина связываются с белками говядины, смягчая жёсткость волокон и раскрывая жирные ноты мяса.' / 'Острота капсаицина в чили активирует тепловые рецепторы, лёгкая горчинка лагера блокирует эти рецепторы и охлаждает нёбо.' / 'Сладость десерта компенсирует горечь дижестива, создавая баланс на финише вечера.' Это поле существует ТОЛЬКО в Эксперт-режиме — в Просто и Стандарт его НЕ заполняй.",
}
...
🚫 НЕВАЛИДНЫЙ ЗАПРОС:
Если пользователь ввёл НЕ блюдо/закуску для человека (корм для животных, несъедобные предметы, бессмысленный текст, только цифры, оскорбления, пустая строка) — верни ТОЛЬКО этот JSON и ничего больше:
{{"error":"Введите название блюда или закуски для человека"}}
...
🚫 НЕВАЛИДНЫЙ ЗАПРОС:
Если пользователь ввёл НЕ напиток (еду, несъедобные предметы, бессмысленный текст, только цифры, оскорбления, пустая строка) — верни ТОЛЬКО этот JSON и ничего больше:
