...
def _build_prompt(req: PairRequest, is_premium: bool = False, recent_brands: list[str] | None = None) -> str:
    ...
    # Секция антиповторов: последние бренды из истории пользователя
    history_section = ""
    if recent_brands:
        unique_brands = list(dict.fromkeys(recent_brands))[:10]
        brands_str = ", ".join(unique_brands)
        history_section = f"\n🚫 АНТИПОВТОР — НЕ РЕКОМЕНДУЙ эти бренды, пользователь уже получал их в недавних подборках: {brands_str}. Выбери ДРУГИЕ бренды из белого списка или другие линейки. Разнообразие критически важно для ценности сервиса.\n"
...
async def pair_stream(request: Request, req: PairRequest):
    ...
    prompt = _build_prompt(req, is_premium=is_premium, recent_brands=recent_brands)
