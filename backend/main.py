...
def _build_prompt(req: PairRequest, is_premium: bool = False, recent_brands: list[str] | None = None, recent_types: list[str] | None = None) -> str:
    ...
    history_parts = []
    if recent_brands:
        unique_brands = list(dict.fromkeys(recent_brands))[:10]
        brands_str = ", ".join(unique_brands)
        history_parts.append(f"Бренды которые уже рекомендовались: {brands_str}. НЕ ПОВТОРЯЙ их — выбери другие.")
    if recent_types:
        ...
        if overused:
            ...
            history_parts.append(f"Категории которые слишком часто повторяются: {types_str}. ИЗБЕГАЙ их — дай пользователю попробовать ДРУГИЕ категории напитков (коктейли, крепкий алкоголь, игристое, саке и т.д.).")
    if history_parts:
    ...
    if req.preferences:
        ...
        if is_premium:
            ...
            preferences_section = f"\n🎯 ПРЕДПОЧТЕНИЯ ПОЛЬЗОВАТЕЛЯ (ОБЯЗАТЕЛЬНО): {pref_list}. ВСЕ 3 напитка должны быть из этих категорий. Пользователь ЯВНО выбрал что он хочет пить — уважай его выбор. Если он выбрал виски, коньяк, текилу — давай виски, коньяк и текилу, а НЕ красное вино и пиво. Изредка (не чаще 1 из 4 запросов) можно заменить ОДНУ позицию на категорию вне предпочтений — как 'открытие', неожиданное сочетание которое пользователь мог не знать. Но это исключение, не правило.\n"
...
⚠️ ПРИОРИТЕТ ПРЕДПОЧТЕНИЙ НАД ТРАДИЦИЕЙ:
Если у пользователя заданы предпочтения — они ВАЖНЕЕ традиционных пейрингов. К стейку классически подают красное вино, но если пользователь выбрал виски и текилу — дай ему виски и текилу к стейку. Хороший пейринг существует в ЛЮБОЙ категории, не только в вине. Твоя задача — найти лучший напиток к блюду ВНУТРИ предпочтений пользователя, а не навязывать "классику".
...
async def pair_stream(request: Request, req: PairRequest):
    ...
    try:
        with conn2.cursor() as cur:
            cur.execute(
                "SELECT results FROM pairings WHERE firebase_uid = %s ORDER BY created_at DESC LIMIT 5",
                (uid,)
            )
